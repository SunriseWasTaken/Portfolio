...
const bootLines = [
  { p: "aria@voss", c: "whoami", out: `${profile.name} — ${profile.role}` },
  { p: "aria@voss", c: "cat status.txt", out: "◦ Open to select engineering & creative-technologist work." },
  { p: "aria@voss", c: "locate", out: `${profile.location} · remote-friendly` },
];
        ...
        <div className="grid grid-cols-1 gap-8 p-6 font-mono text-sm sm:p-8 md:grid-cols-2">
          ...
          <form onSubmit={onSubmit} className="flex flex-col gap-4">
            ...
            {(["name", "email"] as const).map((field) => (
              <label key={field} className="block">
                <span className="mb-1.5 block text-xs text-white/40">
                  {profile.email.split("@")[0]}@guest:~$ set {field}
                </span>
            ...
            <label className="block">
              <span className="mb-1.5 block text-xs text-white/40">
                {profile.email.split("@")[0]}@guest:~$ compose message
              </span>
